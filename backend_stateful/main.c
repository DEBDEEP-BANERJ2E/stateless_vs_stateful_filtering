...
// Declare the prototype of restrict_udp_traffic function
void restrict_udp_traffic(void);

// Function to check the protocol in a PDML file (identify TCP packets)
void check_protocol_in_pdml(const char *pdml_file) {
    ...
    // Flag to determine if TCP is detected (ignore UDP)
    int is_tcp = 0;
                ...
                if (proto->type == XML_ELEMENT_NODE && strcmp((const char *)proto->name, "proto") == 0) {
                    ...
                    if (protocol_name != NULL) {
                        if (strcmp((const char *)protocol_name, "tcp") == 0) {
                            ...
                            is_tcp = 1;  // Flag TCP packet detection
                        }
    ...
    // Apply filtering based on TCP protocol only
    if (is_tcp) {
        setup_stateless_filtering();  // Apply stateless filtering only for TCP
    } else {
        printf("No TCP packets found in PDML. UDP traffic will be restricted.\n");
        restrict_udp_traffic();  // Restrict UDP traffic if no TCP is found
    }
...
// Function to apply stateless filtering for TCP
void setup_stateless_filtering() {
    printf("Applying stateless filtering rules for TCP...\n");
    system("sudo pfctl -F all");  // Flush existing rules
    system("echo \"pass in proto tcp from any to any port 80\" | sudo pfctl -f -");
    printf("Stateless filtering rules applied for TCP.\n");
}

// Function to restrict UDP traffic
void restrict_udp_traffic() {
    printf("Restricting UDP traffic...\n");
    system("sudo pfctl -F all");  // Flush existing rules
    system("echo \"block in proto udp from any to any port 80\" | sudo pfctl -f -");
    printf("UDP traffic has been restricted.\n");
}
...
int main() {
    ...
    // Step 2: Apply Stateless Filtering Based on TCP (UDP will be restricted)
